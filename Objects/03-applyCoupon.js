...
const applyCoupon = (discount, wishList) => {
    return wishList.reduce((output, el) => {
        return output += applyPercentage(el.price, discount);
    }, 0)
}

const applyPercentage = (element, discount) => {
    let percentage = (element/100) * discount
